use s3::{self, bucket, region, credentials};
use bytes::BytesMut;
use prost::Message;

use crate::settings;
use crate::proto::data::{Anime, Source};
...
impl AnimeStore {
    ...
    pub fn new(config: &settings::Storage) -> Result<Self, StoreError> {
        ...
        let creds = credentials::Credentials::new(Some(config.key().to_owned()), Some(config.secret().to_owned()), None, None);
    ...
    pub async fn upload(&self, anime: &Anime, source: Source) -> Result<(), StoreError> {
        ...
        anime.encode(&mut buf).expect("not enough space for encoding");

        let path = storage_path(anime, source);
        self.bucket.put_object(&path, buf.as_ref(), "application/octet-stream").await?;
        Ok(())
    }
}
...
fn storage_path(anime: &Anime, source: Source) -> String {
    ...
    let id = match source {
        Source::Anidb => anime.source.as_ref().and_then(|s| s.anidb_ids.first()).map(|&s| s).unwrap_or(0),
        Source::Unknown => 0,
    };
