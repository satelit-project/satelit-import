...
pub fn importer(intent: ImportIntent) -> impl Future<Item=ImportIntentResult, Error=ImportError> {
    ...
    let ImportIntent {
        id, new_index_url, old_index_url, source: _, reimport_ids
    } = intent;
...
fn download<U>(old_index_url: U, new_index_url: U, settings: &'static settings::Import) -> impl Future<Item=(), Error=ImportError>
    where
        U: AsRef<str> + Send
{
...
fn import(reimport_ids: Vec<i32>, pool: ConnectionPool, settings: &'static settings::Import) -> impl Future<Item = Vec<i32>, Error = ImportError>
{
    let old_extract_path = settings.old_extract_path();
    let new_extract_path = settings.new_extract_path();
